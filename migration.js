...
const MODELS = {
  'tigers': { icon:'🐯', model: 'https://sketchfab.com/models/0db8f285e8514c65a12036fcc8d91d07/embed?autostart=1&ui_infos=0&ui_controls=0&ui_stop=0' },
  'elephants': { icon:'🐘', model: 'https://sketchfab.com/models/a8e7e10f005f4baab0a2f5079d759fcd/embed?autostart=1&ui_infos=0&ui_controls=0&ui_stop=0' },
  'myna': { icon:'🐦', model: 'https://sketchfab.com/models/62d83ecec89a448693525603a5b9af1d/embed?autostart=1&ui_infos=0&ui_controls=0&ui_stop=0' },
  'house-crow': { icon:'🐦‍⬛', model: 'https://sketchfab.com/models/2c8a3414c5a849818dcf8534130ca0df/embed?autostart=1&ui_infos=0&ui_controls=0&ui_stop=0' },
  'indian-grey-hornbill': { icon:'🦜', model: 'https://sketchfab.com/models/4e3419341a0949bb8a30ec53d690eba5/embed?autostart=1&ui_infos=0&ui_controls=0&ui_stop=0' },
  'zebra': { icon:'🦓', model: 'https://sketchfab.com/models/ca72c919f935418484b4f275c149981c/embed?autostart=1&ui_infos=0&ui_controls=0&ui_stop=0' }
};
...
function loadSpeciesData(det){
  ...
  // Remap some names to our 3D model keys if needed
  if(spKey.includes('elephant')) spKey = 'elephants';
  if(spKey.includes('tiger')) spKey = 'tigers';

  const modelInfo = MODELS[spKey] || MODELS['tigers']; // Fallback
  ...
  document.getElementById('mig-status').textContent = 'Loaded from API';
  document.getElementById('mig-desc').textContent = mig.migration_description || '';
  
  if(mig.key_habitats){
    document.getElementById('hab-chips').innerHTML = mig.key_habitats.map(h=>`<span class="hab-chip">${h}</span>`).join('');
  }

  // 3D Model
  document.getElementById('model-loading').style.display='flex';
  document.getElementById('model-iframe').src = modelInfo.model;
  document.getElementById('model-name').textContent = info.common_name || spKey;
  document.getElementById('model-sci').textContent = info.scientific_name || '';

  // Draw Route
  if(routeLine) map.removeLayer(routeLine);
  if(mig.migration_route && mig.migration_route.length > 0){
    const latlngs = mig.migration_route.map(r => [r.lat, r.lng]);
    // Duplicate route for glow effect
    L.polyline(latlngs, {
      color: color,
      weight: 12,
      opacity: 0.15
...
function updateSeasonalPosition(month){
  if(!currentData || !currentData.migration_data) return;
  ...
  // Find pos for month in seasonal_positions
  let pos = null;
  ...
  // Fallback to finding in the route if no seasonal_positions map exists
  ...
  const color = currentData.color_hex || '#F5A623';
  const name = currentData.species_info?.common_name || spKey;

  if(animalMarker) map.removeLayer(animalMarker);
  
  const iconHtml = `<div class="animal-marker" style="border-color:${color};box-shadow:0 0 20px ${color}88;">${modelInfo.icon}</div>`;
  const aIcon = L.divIcon({ html: iconHtml, className: '', iconSize: [40,40], iconAnchor: [20,20] });
  
  animalMarker = L.marker([pos.lat, pos.lng], { icon: aIcon, zIndexOffset: 1000 }).addTo(map);
  
  const popupHtml = `<div class="popup-title">${modelInfo.icon} ${name}</div>
  ...
  animalMarker.bindPopup(popupHtml);
